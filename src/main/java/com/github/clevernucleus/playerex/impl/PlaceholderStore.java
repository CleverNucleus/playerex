...
public final class PlaceholderStore {
	...
	private static NameLevelPair nameLevelPair(final MinecraftServer server, final Collection<String> namesIn, final int indexIn) {
		...
		for(String name : namesIn) {
			names[i++] = new NameLevelPair(name, PlayerCacheAPI.get(server, name, ExAPI.LEVEL_VALUE));
		}
	...
	private static PlaceholderHandler top(Function<NameLevelPair, String> function) {
		return ctx -> {
			...
			Collection<String> names = PlayerCacheAPI.getPlayerNames(server);
			int index = 1;
	...
	static {
		STORE.put(new Identifier(ExAPI.MODID, "level"), ctx -> {
			...
			int level = DataAttributesAPI.ifPresent(player, ExAPI.LEVEL, 0, value -> Math.round(value));
