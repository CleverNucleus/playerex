	...
	public void draw(MatrixStack par0, final boolean par1) {
		...
		if(par1) {
			...
			ElementRegistry.GET_PLAYER_ELEMENTS.apply(var0).ifPresent(var -> {
				int var3 = (int)(182F * Util.expCoeff((float)ElementRegistry.LEVEL.get(var0, var), (float)ElementRegistry.EXPERIENCE.get(var0, var)));
			...
			ElementRegistry.GET_PLAYER_ELEMENTS.apply(var0).ifPresent(var -> {
				...
				if(Screen.hasAltDown()) {
					var8 = (int)ElementRegistry.LEVEL.get(var0, var);
					var9 = 16759296;
				} else {
