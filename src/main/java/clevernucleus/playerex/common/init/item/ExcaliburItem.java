...
public class ExcaliburItem extends SwordItem implements ILoot {
	...
	public void addInformation(ItemStack par0, World par1, List<ITextComponent> par2, ITooltipFlag par3) {
		par2.add(new StringTextComponent(TextFormatting.GRAY + (new TranslationTextComponent(PlayerEx.MODID + ".excalibur.desc")).getString()));
	}
				...
				if(par0 instanceof ServerWorld) {
					...
					for(BlockPos var : positions(var6, var7, var8, var3, var4, var5)) {
						...
						for(int var12 = 0; var12 < 20; var12++) {
							var9.spawnParticle(ParticleTypes.LAVA, var.getX() + Util.randomVe.apply(0.5D), var.getY() + Util.randomVe.apply(0.5D) - 0.25D, var.getZ() + Util.randomVe.apply(0.5D), 1, (random.nextDouble() - 0.5D) * 2.0D, -random.nextDouble(), (random.nextDouble() - 0.5D) * 2.0D, 0.5D);
						}
						
						var9.createExplosion(par1, DamageSource.causeExplosionDamage(par1), null, var.getX(), var.getY(), var.getZ(), 1.0F, false, Explosion.Mode.DESTROY);
					}
				}
