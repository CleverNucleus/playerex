...
public class HealBranchItem extends Item implements ILoot {
	...
	public void addInformation(ItemStack par0, World par1, List<ITextComponent> par2, ITooltipFlag par3) {
		par2.add(new TranslationTextComponent("tooltip.heal_branch", TextFormatting.GRAY));
	}
				...
				if(par0 instanceof ServerWorld) {
					...
					for(LivingEntity var : var7) {
						...
						for(int var12 = 0; var12 < 10; var12++) {
							var6.spawnParticle(ParticleTypes.HEART, var.getPosX() + Util.RANDOM.apply(0.5D), var.getPosY() + var.getEyeHeight() + Util.RANDOM.apply(0.5D) - 0.25D, var.getPosZ() + Util.RANDOM.apply(0.5D), 1, (random.nextDouble() - 0.5D) * 2.0D, -random.nextDouble(), (random.nextDouble() - 0.5D) * 2.0D, 0.5D);
						}
					}
